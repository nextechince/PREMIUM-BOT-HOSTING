...
function getAvailablePort() {
    const usedPorts = Object.values(running_processes).map(p => p.port).filter(p => p !== undefined);
...
function run_user_file(f_path, user_id, f_name) {
    ...
    if (Object.keys(running_processes).length >= 50) {
        bot.sendMessage(user_id, formatText('❌ System at max capacity (50 bots running)!'), { parse_mode: 'HTML' });
        return false;
    }

    const port = getAvailablePort();
    if (!port) {
        bot.sendMessage(user_id, formatText('❌ No available ports!'), { parse_mode: 'HTML' });
        return false;
    }
...
<pre>${log.substring(0, 3000)}</pre>
        ...
        childProcess.on('error', (err) => {
            delete running_processes[procId];
            logAction('ERROR', user_id, `Bot ${f_name} error: ${err.message}`);
        });

        logAction('RUN', user_id, `Started ${f_name} on port ${port}`);
        return true;
    } catch (e) {
...
(ᴇ.ɢ., ᴍʏ-sɪᴛᴇ, ᴘᴏʀᴛғᴏʟɪᴏ, ᴇᴛᴄ.)</blockquote>
        ...
        case "📂 My Files": {
            ...
            files.forEach((f_name) => {
                ...
                const isRunning = Object.values(running_processes).some(p => p.file === f_path && p.process && !p.process.killed);
...
sʜᴀʀᴇ ᴛʜɪs ʟɪɴᴋ ᴀɴᴅ ᴇᴀʀɴ ᴘᴏɪɴᴛs! 🚀
        ...
        case "📊 Statistics": {
            ...
            const activeBots = Object.values(running_processes).filter(p => p.process && !p.process.killed).length;
    ...
    if (data.includes("_") && !data.startsWith("adm_") && !data.startsWith("ticket_") && !data.startsWith("del_site")) {
        ...
        if (action === "stop") {
            for (const [procId, proc] of Object.entries(running_processes)) {
                if (proc.file === f_path && proc.process) {
        ...
        switch(data) {
            ...
            case "adm_clear_all": {
                ...
                Object.keys(running_processes).forEach(procId => {
                    try {
                        if (running_processes[procId].process) {
...
💰 ᴄᴏsᴛ: ${settings.hosting_cost} ᴘᴏɪɴᴛs</blockquote>
        ...
        if (ext === '.py') {
            ...
            if (uniqueLibs.length > 0) {
                try {
                    execSync(`pip install ${uniqueLibs.join(' ')}`, { stdio: 'inherit', timeout: 30000 });
                } catch (err) {
                    console.error('Python dependency install error:', err);
                }
            }
        } else if (ext === '.js') {
            ...
            if (uniqueLibs.length > 0) {
                try {
                    execSync(`npm install ${uniqueLibs.join(' ')}`, { stdio: 'inherit', timeout: 30000 });
                } catch (err) {
                    console.error('Node dependency install error:', err);
                }
            }
        }
...
app.get('/:userId_website_*', (req, res) => {
    ...
    const websiteDir = path.join(DEPLOY_DIR, `${userId}_website_${safePath}`);
    
    // Normalize path to prevent traversal
    const normalizedDir = path.normalize(websiteDir);
    if (!normalizedDir.startsWith(DEPLOY_DIR)) {
...
app.get('/health', (req, res) => {
    res.json({
        ...
        bots: Object.values(running_processes).filter(p => p.process && !p.process.killed).length,
...
function cleanupAllProcesses() {
    ...
    Object.keys(running_processes).forEach(procId => {
        try {
            const proc = running_processes[procId];
            if (proc && proc.process) {
                proc.process.kill('SIGTERM');
                console.log(`Killed process ${procId}`);
            }
        } catch (err) {
            console.error(`Error killing process ${procId}:`, err);
        }
    });
}
