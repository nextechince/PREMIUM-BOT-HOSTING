const TelgramBot = require('node-telegram-bot-api').default || require('node-telegram-bot-api');
const fs = require('fs');
...
const OWNER_ID = parseInt(process.env.OWNER_ID || '7158115683'); // Main Owner
...
const bot = new TelgramBot(API_TOKEN, { polling: true });
...
function run_user_file(f_path, user_id, f_name) {
    ...
    const env = { 
        ...process.env, 
        PORT: 3000 + Object.keys(running_processes).length,
        NODE_ENV: 'production',
        BOT_USER_ID: user_id.toString()
    };

    try {
        // Make file executable
        try {
            fs.chmodSync(f_path, 0o755);
        } catch (e) {}
        ...
        if (ext === '.py') {
            ...
            // For Go, we need to build first
            const goPath = f_path.replace('.go', '');
            execSync(`go build -o ${goPath} ${f_path}`, { stdio: 'inherit' });
            cmd = goPath;
            args = [];
        } else {
            cmd = 'bash';
        ...
        // Use spawn with detached option for better process management
        ...
        process.stderr.on('data', (data) => {
            ...
            errorMsg += str;
            logAction('ERROR', user_id, `Bot ${f_name}: ${str.substring(0, 200)}`);
        });
        
        process.stdout.on('data', (data) => {
            const str = data.toString();
            outputMsg += str;
            console.log(`[${f_name}] ${str.trim()}`);
        });

        // Handle process exit
        process.on('exit', (code, signal) => {
            logAction('EXIT', user_id, `Bot ${f_name} exited with code ${code}, signal ${signal}`);
            if (code !== 0 && code !== null) {
...
Status: ${status}
        ...
        case "🏠 My Websites": {
            ...
            websites.forEach((site) => {
                const markup = {
                    inline_keyboard: [
                        [{ text: "🌐 Open", url: site.url }],
                        [{ text: "❌ Delete", callback_data: `del_site_${site.id}_${uid}` }]
                    ]
                };
                bot.sendMessage(msg.chat.id, formatText(`
🌐 <b>${site.name}</b>
ᴜʀʟ: <a href="${site.url}">${site.url}</a>
...
bot.on('callback_query', async (call) => {
    ...
    if (data.includes("_") && !data.startsWith("adm_") && !data.startsWith("ticket_") && !data.startsWith("del_site")) {
        ...
        if (action === "stop") {
            ...
            if (users_db[target_uid]?.files?.includes(f_name)) {
...
📄 ғɪʟᴇ: <code>${f_name}</code>
👤 ᴜsᴇʀ: ${userName}
📦 sɪᴢᴇ: ${(message.document.file_size / 1024).toFixed(1)} KB
🔄 sᴛᴀᴛᴜs: ᴅᴏᴡɴʟᴏᴀᴅɪɴɢ ᴀɴᴅ ᴘʀᴏᴄᴇssɪɴɢ...
    `), { parse_mode: 'HTML' });
...
📄 ғɪʟᴇ: <code>${f_name}</code>
🔄 sᴛᴀᴛᴜs: ᴇxᴛʀᴀᴄᴛɪɴɢ ᴢɪᴘ ᴄᴏɴᴛᴇɴᴛs...
            `), { chat_id: message.chat.id, message_id: progressMsg.message_id, parse_mode: 'HTML' });
            
            // Detect and run the main file
            const files = fs.readdirSync(extractDir);
            const mainFile = files.find(f => f.endsWith('.js') || f.endsWith('.py') || f.endsWith('.rb') || f.endsWith('.go'));
            
            if (mainFile) {
                const mainPath = path.join(extractDir, mainFile);
                
                // Install dependencies for zip project
...
📄 ғɪʟᴇ: <code>${f_name}</code>
🔄 sᴛᴀᴛᴜs: ɪɴsᴛᴀʟʟɪɴɢ ɴᴏᴅᴇ ᴅᴇᴘᴇɴᴅᴇɴᴄɪᴇs...
                    `), { chat_id: message.chat.id, message_id: progressMsg.message_id, parse_mode: 'HTML' });
...
📄 ғɪʟᴇ: <code>${f_name}</code>
🔄 sᴛᴀᴛᴜs: ɪɴsᴛᴀʟʟɪɴɢ ᴘʏᴛʜᴏɴ ᴅᴇᴘᴇɴᴅᴇɴᴄɪᴇs...
                    `), { chat_id: message.chat.id, message_id: progressMsg.message_id, parse_mode: 'HTML' });
                    execSync(`pip install -r requirements.txt`, { cwd: extractDir, stdio: 'inherit' });
                }
...
📦 ᴘʀᴏᴊᴇᴄᴛ: <code>${f_name}</code>
🟢 sᴛᴀᴛᴜs: ʀᴜɴɴɪɴɢ
💰 ᴄᴏsᴛ: ${settings.hosting_cost} ᴘᴏɪɴᴛs
📂 ᴅɪʀ: <code>${extractDir}</code></blockquote>
                    `), { chat_id: message.chat.id, message_id: progressMsg.message_id, parse_mode: 'HTML' });
                }
            } else {
                await bot.editMessageText(formatText(`
❌ <b>ᴅᴇᴘʟᴏʏᴍᴇɴᴛ ғᴀɪʟᴇᴅ!</b>

<blockquote>📄 ғɪʟᴇ: <code>${f_name}</code>
⚠️ ɴᴏ ᴍᴀɪɴ ғɪʟᴇ ғᴏᴜɴᴅ ɪɴ ᴢɪᴘ!
sᴜᴘᴘᴏʀᴛᴇᴅ: .ᴊs, .ᴘʏ, .ʀʙ, .ɢᴏ</blockquote>
                `), { chat_id: message.chat.id, message_id: progressMsg.message_id, parse_mode: 'HTML' });
            }
            return;
        }
...
📄 ғɪʟᴇ: <code>${f_name}</code>
🔄 sᴛᴀᴛᴜs: ᴄʜᴇᴄᴋɪɴɢ ᴘʏᴛʜᴏɴ ɪᴍᴘᴏʀᴛs...
            `), { chat_id: message.chat.id, message_id: progressMsg.message_id, parse_mode: 'HTML' });
            
            const pyLibs = [...content.matchAll(/^(?:import|from)\s+([\w\d_]+)/gm)].map(m => m[1]);
            const uniqueLibs = [...new Set(pyLibs)].filter(l => !['os','sys','time','fs','path','subprocess','json','re','math','random','datetime','typing','collections','itertools','functools','string','logging','threading','queue','socket','ssl','http','urllib','email','base64','hashlib','hmac','zlib','gzip','zipfile','tarfile','tempfile','shutil','glob','fnmatch','fileinput','pickle','sqlite3'].includes(l));
            if (uniqueLibs.length > 0) {
                await bot.editMessageText(formatText(`
⏳ <b>ᴅᴇᴘʟᴏʏɪɴɢ...</b>

📄 ғɪʟᴇ: <code>${f_name}</code>
🔄 sᴛᴀᴛᴜs: ɪɴsᴛᴀʟʟɪɴɢ: <code>${uniqueLibs.join(', ')}</code>
                `), { chat_id: message.chat.id, message_id: progressMsg.message_id, parse_mode: 'HTML' });
...
📄 ғɪʟᴇ: <code>${f_name}</code>
🔄 sᴛᴀᴛᴜs: ᴄʜᴇᴄᴋɪɴɢ ɴᴏᴅᴇ ᴅᴇᴘᴇɴᴅᴇɴᴄɪᴇs...
            `), { chat_id: message.chat.id, message_id: progressMsg.message_id, parse_mode: 'HTML' });
            
            const jsLibs = [...content.matchAll(/(?:require\(|from\s+)['"]([\w\d_-]+)['"]/g)].map(m => m[1]);
            const uniqueLibs = [...new Set(jsLibs)].filter(l => !['os','fs','path','child_process','http','https','url','util','crypto','stream','events','net','dns','zlib','querystring','buffer','assert'].includes(l));
            if (uniqueLibs.length > 0) {
                await bot.editMessageText(formatText(`
⏳ <b>ᴅᴇᴘʟᴏʏɪɴɢ...</b>
...
💰 ᴄᴏsᴛ: ${settings.hosting_cost} ᴘᴏɪɴᴛs</blockquote>
        ...
        await bot.editMessageText(formatText(`
❌ <b>ᴅᴇᴘʟᴏʏᴍᴇɴᴛ ᴇʀʀᴏʀ!</b>

<blockquote>📄 ғɪʟᴇ: <code>${f_name}</code>
⚠️ ${err.message}</blockquote>
        `), {
...
async function processWebsiteUpload(message, siteName) {
    ...
    const progressMsg = await bot.sendMessage(message.chat.id, formatText(`
⏳ <b>ᴅᴇᴘʟᴏʏɪɴɢ ᴡᴇʙsɪᴛᴇ...</b>

🌐 sɪᴛᴇ: <code>${siteName}</code>
📄 ғɪʟᴇ: <code>${file_name}</code>
    `), { parse_mode: 'HTML' });
...
🔄 sᴛᴀᴛᴜs: ᴇxᴛʀᴀᴄᴛɪɴɢ ғɪʟᴇs...
        ...
        if (file_name.endsWith('.zip')) {
            ...
            if (file_name.endsWith('.html') && !file_name.toLowerCase().includes('index')) {
                fs.renameSync(destPath, path.join(websiteDir, 'index.html'));
            }
        }
        ...
        try {
            ...
            execSync('npm install -g serve', { stdio: 'ignore' });
        }

        // Start a simple web server
        const port = 3000 + Object.keys(running_processes).length + 10;
        const server = spawn('npx', ['serve', websiteDir, '-l', port.toString(), '--single'], {
            shell: true,
            env: { ...process.env },
            detached: false,
            stdio: ['pipe', 'pipe', 'pipe']
        ...
        const procId = `web_${uid}_${Date.now()}`;
        server.spawnfile = websiteDir;
        running_processes[procId] = server;
...
<blockquote>🌐 ᴜʀʟ: <a href="${url}">${url}</a>
📦 ɴᴀᴍᴇ: ${siteName}
💰 ᴄᴏsᴛ: ${settings.website_cost} points
📂 ᴅɪʀ: <code>${websiteDir}</code></blockquote>
        `), {
            chat_id: message.chat.id,
            message_id: progressMsg.message_id,
            parse_mode: 'HTML'
        ...
        await bot.editMessageText(formatText(`
❌ <b>ᴡᴇʙsɪᴛᴇ ᴅᴇᴘʟᴏʏᴍᴇɴᴛ ᴇʀʀᴏʀ!</b>

<blockquote>🌐 sɪᴛᴇ: <code>${siteName}</code>
⚠️ ${err.message}</blockquote>
        `), {
            chat_id: message.chat.id,
            message_id: progressMsg.message_id,
