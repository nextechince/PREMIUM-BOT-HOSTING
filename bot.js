...
const RAILWAY_URL = process.env.RAILWAY_URL || 'https://p-h.up.railway.app'; // SET THIS IN ENV
...
Status: ${status}
        ...
        case "🏠 My Websites": {
            ...
            websites.forEach((site) => {
                const fullUrl = site.url.startsWith('http') ? site.url : `${RAILWAY_URL}/${site.url}`;
                const markup = {
                    inline_keyboard: [
...
async function processWebsiteUpload(message, siteName) {
    ...
    try {
        ...
        // Generate unique subpath
        const subpath = `${uid}_${siteName}`;
        const fullUrl = `${RAILWAY_URL}/${subpath}`;
        
        // Save website info with subpath
...
// Route for deployed websites
app.get('/:userId_website_*', (req, res) => {
    const filePath = path.join(DEPLOY_DIR, req.params[0]);
