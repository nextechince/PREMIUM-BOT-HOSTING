// bot.js - Complete Telegram Bot Panel with Admin Features
// Node.js version using node-telegram-bot-api
...
function sendBotStartedAnnouncement(userId, botName, botId) {
    ...
    try {
        bot.sendMessage(CONFIG.announceChannel, msg, { parse_mode: 'HTML' });
    } catch {}
}
...
const CAPTCHA_POOL = 'ABCDEFGHJKLMNPRSTUVWXYZ23456789';
...
function generateMathCaptcha() {
    ...
    else { answer = a * b; displayOp = '×'; }

    const distractors = new Set();
    distractors.add(answer);
    while (distractors.size < 5) {
        let d = answer + Math.floor(Math.random() * 10) - 5;
...
function startCaptcha(chatId, userId) {
    ...
    captcha.options.forEach(opt => {
        row.push({ text: String(opt), callback_data: `captcha_${opt}` });
    });
    // Split into rows of 3
    for (let i = 0; i < row.length; i += 3) {
        kb.inline_keyboard.push(row.slice(i, i + 3));
    }
    kb.inline_keyboard.push([
        { text: '🔄 New Captcha', callback_data: 'captcha_new' }
    ]);
    ...
    return bot.sendMessage(chatId,
        ...
        `━━━━━━━━━━━━━━━━━━━━━━━\n` +
        `Tap the correct answer below. You have 3 tries.`,
        {
            parse_mode: 'HTML',
            reply_markup: kb,
        }
...
function loadRequiredGroups() {
    return new Promise((resolve) => {
        getSetting('required_groups', []).then(groups => {
...
function checkGroupMembership(uid) {
    return new Promise(async (resolve) => {
        if (uid === CONFIG.ownerId && CONFIG.ownerId > 0) return resolve([]);
        if (await isAdmin(uid)) return resolve([]);

        const notJoined = [];
        for (const grp of requiredGroups) {
            try {
                const member = await bot.getChatMember(grp.id, uid);
...
function sendJoinVerification(chatId, uid, notJoined) {
    const kb = {
        inline_keyboard: []
    };
    notJoined.forEach(grp => {
        kb.inline_keyboard.push([
            { text: `📢 Join ${grp.name}`, url: grp.link }
        ]);
    });
    kb.inline_keyboard.push([
        { text: '✅ Verify Membership', callback_data: 'group_verify_check' }
    ]);

    const msg =
        `<b>🛡️ Group Join Required</b>\n` +
        `━━━━━━━━━━━━━━━━━━━━━━━\n` +
        `You must join the following groups to use this bot:\n` +
        notJoined.map(g => `• <a href="${g.link}">${esc(g.name)}</a>`).join('\n') +
        `\n━━━━━━━━━━━━━━━━━━━━━━━\n` +
...
function requireGroupMembership(chatId, uid) {
    return new Promise(async (resolve) => {
        const notJoined = await checkGroupMembership(uid);
        if (notJoined.length === 0) return resolve(true);
        await sendJoinVerification(chatId, uid, notJoined);
        resolve(false);
    });
}
...
function isVerified(uid) {
    return new Promise((resolve) => {
        if (uid === CONFIG.ownerId && CONFIG.ownerId > 0) return resolve(true);
        db.get('SELECT verified FROM users WHERE id = ?', [uid], (err, row) => {
            resolve(row ? !!row.verified : false);
        });
    });
}
...
function markVerified(uid) {
    return new Promise((resolve) => {
        db.run(`UPDATE users SET verified = 1, verified_at = ? WHERE id = ?`,
            [tsIso(), uid], () => resolve());
    });
}

function requireVerified(chatId, uid) {
    return new Promise(async (resolve) => {
        if (await isVerified(uid)) return resolve(true);

        // Check if captcha session exists
        const session = captchaSessions.get(uid);
        if (session && Date.now() - session.ts < 60000) {
            return resolve(false);
        }

        // Start captcha
        await startCaptcha(chatId, uid);
        resolve(false);
    });
}
...
function maybeAutoBan(uid, reason) {
    // Simple auto-ban after 5 violations
...
function mainMenuKb(admin = false) {
    const kb = {
        inline_keyboard: [
            ...
            [
                { text: `Help`, callback_data: 'menu_help', style: 'primary' },
                { text: `Support`, callback_data: 'menu_support', style: 'danger' },
            ],
            [
                { text: ` My Stats`, callback_data: 'menu_stats', style: 'primary' },
            ],
        ]
    };
    if (admin) {
...
function backMainKb() {
    return {
        inline_keyboard: [
            [{ text: `${G.back}  Main Menu`, callback_data: 'menu_main', style: 'danger' }]
        ]
    };
}
...
function backKb(target, label = 'Back') {
    return {
        inline_keyboard: [
            [{ text: `${G.back}  ${label}`, callback_data: target, style: 'danger' }]
        ]
    };
}
    ...
    for (const [key, val] of Object.entries(PLAN_LIMITS)) {
        ...
        kb.inline_keyboard.push([
            { text: `${G.star}  ${val.name}  ${G.bullet}  ${price}`,
                callback_data: `plan_view_${key}`, style: style }
        ]);
    }
    kb.inline_keyboard.push([
        { text: `${G.back}  Main Menu`, callback_data: 'menu_main', style: 'danger' }
...
function paymentsKb(plan = null) {
    ...
    for (const [key, val] of Object.entries(PAYMENT_METHODS)) {
        kb.inline_keyboard.push([
            { text: `${val.tag}  ${val.name}`, callback_data: `pay_${key}${suffix}`, style: 'success' }
        ]);
    }
    kb.inline_keyboard.push([
        { text: `${G.back}  Plans`, callback_data: 'menu_plans', style: 'primary' }
    ]);
    return kb;
}
    ...
    return {
        inline_keyboard: [
            [
                ...
                { text: `${G.users}  Users`, callback_data: 'adm_users', style: 'primary' },
            ],
            [
                { text: `${G.diamond}  All Bots`, callback_data: 'adm_allbots', style: 'primary' },
                { text: `${G.wallet}  Payments`, callback_data: 'adm_payments', style: 'success' },
            ],
            [
                { text: `${G.broadcast}  Broadcast`, callback_data: 'adm_broadcast', style: 'success' },
                { text: `${G.no}  Ban / Unban`, callback_data: 'adm_ban', style: 'danger' },
            ],
            [
                { text: `${G.plus}  Give Plan`, callback_data: 'adm_giveplan', style: 'success' },
                { text: `${G.ok}  Approve Pay`, callback_data: 'adm_approve', style: 'success' },
            ],
            [
                { text: `${G.key}  Coupons`, callback_data: 'adm_coupons', style: 'primary' },
                { text: `${G.ticket}  Tickets`, callback_data: 'adm_tickets', style: 'primary' },
            ],
            [
                { text: `${G.shield}  Admins`, callback_data: 'adm_admins', style: 'primary' },
                { text: `${G.eye}  Audit Log`, callback_data: 'adm_audit', style: 'primary' },
            ],
            [
            ...
            [
                { text: `${G.warn}  Maintenance`, callback_data: 'adm_maint', style: 'danger' },
                { text: `${G.settings}  Settings`, callback_data: 'adm_settings', style: 'primary' },
            ],
            [
            ...
            [
                { text: `💬  Notifications`, callback_data: 'adm_notify_center', style: 'success' },
                { text: `⚙️  Sys Tools`, callback_data: 'adm_sys_tools', style: 'primary' },
            ],
            [
                { text: `🐙  Gh Browser`, callback_data: 'adm_gh_browser', style: 'primary' },
                { text: `💳  Pay Config`, callback_data: 'adm_pay_config', style: 'success' },
            ],
            [
            ...
            [
                ...
                { text: `📝  Templates`, callback_data: 'adm_templates', style: 'primary' },
            ],
            [
                { text: `🔗  Referral Sys`, callback_data: 'adm_referral_sys', style: 'success' },
                { text: `🧹  Janitor`, callback_data: 'adm_janitor', style: 'danger' },
            ],
            [
                { text: `🌐  Webhooks`, callback_data: 'adm_webhooks', style: 'primary' },
                { text: `🎯  Feature Flags`, callback_data: 'adm_feature_flags', style: 'primary' },
            ],
            [
                { text: `⏱️  Rate Limits`, callback_data: 'adm_rate_config', style: 'danger' },
                { text: `📡  Live Monitor`, callback_data: 'adm_live_monitor', style: 'success' },
            ],
            [
                { text: `💎  Rev Goals`, callback_data: 'adm_rev_goals', style: 'success' },
                { text: `⏰  Scheduler`, callback_data: 'adm_scheduler', style: 'primary' },
            ],
            [
            ...
            [
                { text: `${G.back}  Main Menu`, callback_data: 'menu_main', style: 'primary' }
            ],
        ]
    };
}
...
function botActionsKb(botId, running, premium = false) {
    ...
    if (running) {
        kb.inline_keyboard.push([
            ...
            { text: `${G.refresh}  Restart`, callback_data: `bot_restart_${botId}`, style: 'success' },
        ]);
    } else {
        kb.inline_keyboard.push([
            { text: `${G.play}  Start`, callback_data: `bot_start_${botId}`, style: 'success' },
    ...
    kb.inline_keyboard.push([
        ...
        { text: `${G.eye}  Info`, callback_data: `bot_info_${botId}`, style: 'primary' },
    ]);
    kb.inline_keyboard.push([
        { text: `${G.settings}  Env Vars`, callback_data: `bot_env_${botId}`, style: 'primary' },
        { text: `${G.cog}  Cron`, callback_data: `bot_cron_${botId}`, style: 'primary' },
    ]);
    kb.inline_keyboard.push([
        { text: `${G.download}  Install Pkg`, callback_data: `bot_pip_${botId}`, style: 'primary' },
        { text: `${G.plus}  Clone`, callback_data: `bot_clone_${botId}`, style: 'primary' },
    ]);
    if (premium) {
        kb.inline_keyboard.push([
            { text: `🌐  Public URL`, callback_data: `bot_tunnel_${botId}`, style: 'success' },
        ]);
    }
    kb.inline_keyboard.push([
    ...
    kb.inline_keyboard.push([
        { text: `${G.back}  My Bots`, callback_data: 'menu_bots', style: 'primary' }
    ]);
    return kb;
}
...
function confirmKb(yesCb, noCb = 'menu_main', yesLabel = 'Confirm', noLabel = 'Cancel') {
    return {
        inline_keyboard: [
            [
                { text: `${G.ok}  ${yesLabel}`, callback_data: yesCb, style: 'success' },
                { text: `${G.no}  ${noLabel}`, callback_data: noCb, style: 'danger' },
            ]
        ]
    };
}
...
function loading(call, label = 'Loading') {
    ...
    const animate = () => {
        ...
        for (const step of steps) {
            ...
            pct = step;
            render();
            // Wait 0.7s between steps
            const start = Date.now();
            while (Date.now() - start < 700) {
                if (stopFlag.stop) return;
                // busy wait is fine in this context
            }
        }
    };
...
async function renderMainMenu(chatId, uid, call = null, intro = null) {
    ...
    return showMenu(chatId, 'https://placehold.co/900x460/1E1B4B/FFFFFF?text=Main+Menu', cap, mainMenuKb(admin), call);
}
...
function detectEntry(botDir) {
    ...
    // Recursive scan for any .py file
...
function childStatus(botId, botDoc) {
    ...
    if (running && entry && entry.proc) {
        try {
            // Simple CPU/Memory estimation (works on Linux)
...
bot.onText(/\/start/, async (msg) => {
    ...
    if (!await requireVerified(msg.chat.id, uid)) {
        return;
    }

    // Group membership verification
    if (!await requireGroupMembership(msg.chat.id, uid)) {
        return;
    }
...
bot.onText(/\/help/, async (msg) => {
    if (msg.chat.type !== 'private') return;
    if (await bannedBlock(msg)) return;
    if (!await requireVerified(msg.chat.id, msg.from.id)) return;
...
bot.on('document', async (msg) => {
    ...
    await getOrCreateUser(msg.from);
    if (!await requireVerified(msg.chat.id, uid)) return;
...
async function handleBotUpload(msg) {
    ...
    try {
        ...
        for (const f of filesAdded) {
            ...
            // Store key (in memory only for now)
            // In production, use GitHub keyring
...
async function handlePaymentProof(msg, st) {
    ...
    const kb = {
        inline_keyboard: [
            [
                { text: `${G.ok}  Approve`, callback_data: `payapprove_${pid}`, style: 'success' },
...
function replaceMenuPhoto(key, fileBytes) {
    ...
    try {
        ...
        // Update PHOTOS mapping to use local file
...
async function routeCallback(call, data) {
    ...
    if (data.startsWith('captcha_')) {
        handleCaptchaCallback(call);
        return;
    }

    // ---- Group verification ----
    if (data === 'group_verify_check') {
        handleGroupVerify(call);
        return;
    }
...
async function handleCaptchaCallback(call) {
    const uid = call.from.id;
    const chatId = call.message.chat.id;
    const data = call.data;
    ...
    if (data === 'captcha_new') {
        ...
        const session2 = captchaSessions.get(uid) || {};
        session2.regens = (session2.regens || 0) + 1;
        captchaSessions.set(uid, session2);
        await startCaptcha(chatId, uid);
        return;
    }
    ...
    if (answer === session.answer) {
        captchaSessions.delete(uid);
        await markVerified(uid);
        ack(call, '✓ Verified');
        try {
            await bot.deleteMessage(chatId, call.message.message_id);
        } catch {}
        audit(uid, 'captcha_pass', `tries=${session.tries || 0}`);
        renderMainMenu(chatId, uid, null,
            `<b>${G.ok} Verification complete</b> — welcome, <b>${esc(call.from.first_name || 'friend')}</b>!`
        );
        return;
    }

    session.tries = (session.tries || 0) + 1;
    const left = Math.max(0, 3 - session.tries);
    ...
    if (session.tries >= 3) {
        ...
        ack(call, `Wrong. ${left} try(s) left.`);
    }
}

// ============================================================
// GROUP VERIFY CALLBACK
// ============================================================

async function handleGroupVerify(call) {
    const uid = call.from.id;
    const chatId = call.message.chat.id;
...
async function renderBotsMenu(call) {
    ...
    kb.inline_keyboard.push([
        { text: `${G.plus}  Upload`, callback_data: 'menu_upload', style: 'success' },
        { text: `${G.back}  Main Menu`, callback_data: 'menu_main', style: 'primary' },
    ]);
...
async function renderBotDeleteConfirm(call, botId) {
    ...
    const kb = {
        inline_keyboard: [
            [
                { text: `${G.ok}  Delete Record`, callback_data: `bot_delyes_${botId}`, style: 'danger' },
                { text: `${G.no}  Cancel`, callback_data: `bot_view_${botId}`, style: 'primary' },
            ]
        ]
    };
...
async function renderBotDelfilesConfirm(call, botId) {
    ...
    const kb = {
        inline_keyboard: [
            [
                { text: `${G.ok}  Delete Files`, callback_data: `bot_delfilesyes_${botId}`, style: 'danger' },
                { text: `${G.no}  Cancel`, callback_data: `bot_view_${botId}`, style: 'primary' },
            ]
        ]
    };
...
async function renderBotDelallConfirm(call, botId) {
    ...
    const kb = {
        inline_keyboard: [
            [
                { text: `${G.no}  Delete All`, callback_data: `bot_delalyes_${botId}`, style: 'danger' },
                { text: `${G.ok}  Cancel`, callback_data: `bot_view_${botId}`, style: 'primary' },
            ]
        ]
    };
...
async function startTunnelFlow(call, botId) {
    ...
    bot.sendMessage(call.message.chat.id,
        ...
        `${G.div}\n` +
        `Send the local port your bot is listening on (e.g. <code>8080</code>).\n` +
...
async function renderAdminSubroute(call, data) {
    ...
    if (data === 'adm_tg_backup') return renderAdminTgBackup(call);
    if (data === 'adm_gh_browser') return renderAdminGhBrowser(call);
    if (data === 'adm_approval_group') return renderAdminApprovalGroup(call);
    if (data === 'adm_private_group') return renderAdminPrivateGroup(call);
    ...
    // Sub-route callbacks
    if (data === 'adm_mass_restart_stopped') return renderAdminMassRestartStopped(call);
    if (data === 'adm_mass_restart_stopped_yes') return actionAdminMassRestartStopped(call);
    if (data === 'adm_kill_all_now') return renderAdminConfirmCustom(call, 'adm_kill_all_now_yes', 'Kill ALL running bots immediately', 'adm_bot_manager');
    if (data === 'adm_kill_all_now_yes') return actionAdminKillAll(call);
    if (data.startsWith('adm_notify_plan_')) {
        const plan = data.split('_')[3];
        return renderAdminNotifyPlan(call, plan);
    }
    if (data === 'adm_notify_plan_select') return renderAdminNotifyPlanSelect(call);
    if (data === 'adm_clear_cache') {
        ack(call, 'Caches cleared!');
        return renderAdminSysTools(call);
    }
...
async function renderAdminUsers(call) {
    ...
    showMenu(call.message.chat.id, PHOTOS.admin, cap, backAdminKb(), call);
}
...
bot.on('text', async (msg) => {
    ...
    if (text.startsWith('/')) return; // command handlers
    ...
    try {
        ...
        if (flow === 'await_payment_proof') {
            // If text contains numbers, try to parse as amount
            const match = text.match(/\d+/);
            if (match) {
                const amount = parseInt(match[0]);
                // Create a fake message with caption
...
async function renderAdminPhotos(call) {
    ...
    const cap =
        ...
        `${G.div_eq}\n` +
        `Tap any menu below, then send a photo to replace its banner.` +
        FOOTER;

    const kb = { inline_keyboard: [] };
    for (const [key, label] of Object.entries(PHOTO_KEYS_FRIENDLY)) {
        kb.inline_keyboard.push([
            { text: `${G.cog}  ${label}`, callback_data: `adm_photo_${key}` }
        ]);
    }
    kb.inline_keyboard.push([
...
function janitorCleanup() {
    setInterval(async () => {
        try {
            ...
            // Clean orphan sandbox directories
...
loadRequiredGroups().then(() => {
    console.log('[startup] Required groups loaded');
});
