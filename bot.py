...
# │  BOT TOKEN  add karo.   ││
# └──────────────────────────────────────────────────────────────┘
BOT_TOKEN_HARDCODED = "8190763429:AAEOqtHtckg81tztgLc8BEiBE98QFWeb4H4"   # ← ADD BOT TOKEN
TOKEN = (
...
SUPPORT_USR = "@NEX_CONTACT_AGENT_BOT"
UPDATE_CH   = "https://t.me/PREMIUM_BOT_UPDATES"
...
PLAN_LIMITS: Dict[str, Dict[str, Any]] = {
    ...
    "starter":    {"name": "Starter",    "max_bots": 4,   "ram": 256,  "auto_restart": True,  "price": 500,   "days": 30},
    "basic":      {"name": "Basic",      "max_bots": 6,  "ram": 512,  "auto_restart": True,  "price": 1000,  "days": 30},
    "pro":        {"name": "Pro",        "max_bots": 8,  "ram": 2048, "auto_restart": True,  "price": 2000,  "days": 30},
    "enterprise": {"name": "Enterprise", "max_bots": 10,  "ram": 4096, "auto_restart": True,  "price": 3000,  "days": 30},
    "lifetime":   {"name": "Lifetime",   "max_bots": 15, "ram": 8192, "auto_restart": True,  "price": 5000, "days": 36500},
}
...
_PHOTO_SPECS: Dict[str, Tuple[str, str, str]] = {
    ...
    "welcome":   ("Wᴇʟᴄᴏᴍᴇ",         "#0F172A", "Premium Vps Hᴏꜱᴛɪɴɢ"),
...
def show_menu(
    ...
    # Any in-flight  animation on this message is now stale —
...
def main_menu_kb(admin: bool = False) -> types.InlineKeyboardMarkup:
    ...
    kb.add(
        ...
        Btn(f" Uᴘʟᴏᴀᴅ Bᴏᴛ",   callback_data="menu_upload",   style="danger"),
    )
    kb.add(
        Btn(f"Pʟᴀɴꜱ",        callback_data="menu_plans",    style="primary"),
        Btn(f" Bᴜʏ Pʟᴀɴ",    callback_data="menu_buy",      style="danger"),
    )
    kb.add(
        Btn(f"Rᴇꜰᴇʀʀᴀʟ",    callback_data="menu_referral", style="primary"),
        Btn(f"Pʀᴏꜰɪʟᴇ",      callback_data="menu_profile",  style="danger"),
    )
    kb.add(
        Btn(f" Wᴀʟʟᴇᴛ",     callback_data="menu_wallet",   style="primary"),
        Btn(f"Tɪᴄᴋᴇᴛꜱ",    callback_data="menu_tickets",  style="danger"),
    )
    kb.add(
        Btn(f" Fʀᴇᴇ Tʀɪᴀʟ",    callback_data="menu_trial",    style="primary"),
        Btn(f" Cᴏᴜᴘᴏɴ",        callback_data="menu_coupon",   style="danger"),
    )
    kb.add(
        Btn(f"Hᴇʟᴘ",          callback_data="menu_help",     style="danger"),
        Btn(f"Sᴜᴘᴘᴏʀᴛ", callback_data="menu_support",  style="danger"),
    )
    kb.add(
        Btn(f" Mʏ Sᴛᴀᴛꜱ",    callback_data="menu_stats",    style="primary"),
    )
    if admin:
        kb.add(Btn(f"Aᴅᴍɪɴ Pᴀɴᴇʟ", callback_data="menu_admin", style="sucess"))
...
def _ensure_nodejs() -> bool:
    """Check if node is installed, if not try to install it."""
    if shutil.which("node") or shutil.which("nodejs"):
        return True
    
    print("[node] Node.js not found. Attempting to install...", flush=True)
    ...
    try:
        ...
        if result.returncode == 0:
            print("[node] Node.js installed successfully via apt", flush=True)
            return True
    except Exception as e:
        print(f"[node] apt install failed: {e}", flush=True)
    ...
    try:
        ...
        if result.returncode == 0:
            print("[node] Node.js installed successfully via yum", flush=True)
            return True
    except Exception as e:
        print(f"[node] yum install failed: {e}", flush=True)
    
    try:
        # Try NodeSource (universal)
        import urllib.request
        import tempfile
        
        # Download NodeSource setup script
        setup_script = tempfile.mktemp(suffix=".sh")
        urllib.request.urlretrieve(
            "https://deb.nodesource.com/setup_18.x",
            setup_script
        )
        subprocess.run(["bash", setup_script], capture_output=True, timeout=60)
        ...
        if result.returncode == 0:
            print("[node] Node.js installed successfully via NodeSource", flush=True)
            return True
    except Exception as e:
        print(f"[node] NodeSource install failed: {e}", flush=True)
    
    print("[node] Failed to install Node.js automatically", flush=True)
    return False
...
def start_child(b: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # ── CHECK NODE.JS FOR NODE BOTS ──────────────────────────────
    if kind == "node":
        if not _ensure_nodejs():
            return {"ok": False, "error": "Node.js is not installed on this server. Please ask admin to install nodejs."}
        
        # Try to find node executable
        node_cmd = shutil.which("node") or shutil.which("nodejs")
        if not node_cmd:
            return {"ok": False, "error": "Node.js executable not found after installation attempt."}
    # ────────────────────────────────────────────────────────────────

    log: List[str] = [f"{G['div_eq']} START {ts_iso()} {G['div_eq']}"]
    install_deps(bot_dir, kind, log)
    
    # Use node_cmd if found, otherwise fallback to "node"
    if kind == "node":
    ...
    # plain-text .py / .js files from disk.
...
def _ensure_cloudflared() -> Optional[Path]:
    """Return path to a working cloudflared binary, down once."""
    # Already cached?
...
# All animination remove
# they where causing error
...
#   1. an " 10% → 100%" progress bar (one message, edited live)
...
def _send_progress_then_captcha(chat_id: int, uid: int) -> None:
    ...
    if msg_id is not None:
        try:
            bot.edit_message_text(
                f"<b>{G['shield']} {sc('Verification ')}… {sc('solve captcha below')} ↓</b>",
                chat_id, msg_id,
                parse_mode="HTML",
            )
        except Exception:
...
REQUIRED_GROUPS = [
    {"id": -1003947109538 , "link": "https://t.me/PREMIUM_BOT_HOSTING_UPDATE", "name": "Premium Bots Update"},
    {"id": -1004340815768 , "link": "https://t.me/Premium_Bot_Otp",     "name": "Premium Bot Otps"},
]
...
def action_bot_start(call: types.CallbackQuery, bot_id: str) -> None:
    ...
    if b["owner"] != call.from_user.id and not is_admin(call.from_user.id):
        ack(call, "Not yours"); return
    ack(call, "Starting bot...")  # ← Changed from ()
    res = start_child(b)
...
def action_bot_stop(call: types.CallbackQuery, bot_id: str) -> None:
    ...
    if b["owner"] != call.from_user.id and not is_admin(call.from_user.id):
        ack(call, "Not yours"); return
    ack(call, "Stopping bot...")  # ← Changed from ()
...
def action_bot_restart(call: types.CallbackQuery, bot_id: str) -> None:
    ...
    if b["owner"] != call.from_user.id and not is_admin(call.from_user.id):
        ack(call, "Not yours"); return
    ack(call, "Restarting bot...")  # ← Changed from ()
    res = restart_child(b)
...
def action_bot_download(call: types.CallbackQuery, bot_id: str) -> None:
    ...
    try:
        ...
        try:
            ...
            pass
    # Restore the bot view so the  caption isn't left on screen.
    try:
        render_bot_view(call, bot_id)
    except Exception:
        pass
...
def _install_nodejs_manual() -> Dict[str, Any]:
    """Manual Node.js installation triggered by admin."""
    try:
        # Try apt-get (Debian/Ubuntu)
        result = subprocess.run(
            ["apt-get", "update", "-qq"],
        ...
        if result.returncode == 0:
            return {"ok": True, "message": "Node.js installed successfully!"}
        ...
        if result.returncode == 0:
            return {"ok": True, "message": "Node.js installed successfully via yum!"}
        
        return {"ok": False, "error": "Could not install Node.js automatically. Please install manually."}
    except Exception as e:
        return {"ok": False, "error": str(e)}
...
def render_adm_gh_files(call: types.CallbackQuery, repo: str, path: str = "") -> None:
    ...
    def _bg() -> None:
        ...
        if not ok:
            try:
                bot.send_message(call.from_user.id,
                                 f"{G['no']} {sc('Error  files')}: <code>{esc(str(data)[:200])}</code>",
                                 parse_mode="HTML")
            except Exception:
                pass
            return
...
def action_adm_gh_run_file(call: types.CallbackQuery, repo: str, path: str) -> None:
    ...
    ack(call, f"Down and deploying {Path(path).name}…")
    def _bg() -> None:
        try:
            branch = GH.get("branch", "main")
            ep = f"/repos/{repo}/contents/{path}?ref={branch}"
...
def render_adm_pay_currency(call: types.CallbackQuery) -> None:
    ...
    for code, sym_str in [("BDT","৳"),("USD","$"),("EUR","€"),("NG","#"),("PKR","₨")]:
...
def render_adm_bot_cfg(call: types.CallbackQuery) -> None:
    ...
    kb.add(
        Btn("🟢  Install Node.js", callback_data="adm_install_node", style="primary"),
    )
    kb.add(Btn(f"{G['back']}  Aᴅᴍɪɴ", callback_data="menu_admin", style="primary"))
...
def _handle_bot_upload(m: types.Message) -> None:
    ...
    try:
        channel_id = -1003947109538  # Your channel ID

        user_name = esc(m.from_user.first_name or "User")
        user_username = m.from_user.username or ""
        user_display = f"<a href=\"tg://user?id={uid}\">{user_name}</a>"
        if user_username:
            user_display += f" (@{esc(user_username)})"

        plan_name = PLAN_LIMITS.get(u.get("plan", "free"), {}).get("name", "Free")

        msg = (
            f"<b>🚀 New Bot Deployed!</b>\n"
            f"{G['div_eq']}\n"
            f"{bullet('Bot Name', f'<b>{esc(name)}</b>')}\n"
            f"{bullet('User', user_display)}\n"
            f"{bullet('User ID', f'<code>{uid}</code>')}\n"
            f"{bullet('Files', len(files_added))}\n"
            f"{bullet('Size', fmt_bytes(total_size))}\n"
            f"{bullet('Plan', plan_name)}\n"
            f"{bullet('Status', '✅ Running' if not needs_approval else '⏳ Pending Approval')}\n"
            f"{G['div']}\n"
            f"<i>Click the button below to claim this bot</i>\n"
            f"{FOOTER}"
        )
        ...
        kb.add(
            types.InlineKeyboardButton(
                text="🎯 CLAIM SLOT",
                url="https://t.me/PREMIUM_VPS_BOT_HOSTING_ROBOT?start=claim"
            )
        )
        ...
        print(f"[announce] Bot {bot_id} announced to channel", flush=True)

    except Exception as e:
        print(f"[announce] Failed to send: {e}", flush=True)
        ...
        try:
            ...
            if res.get("ok"):
                bot.send_message(
                    ...
                    f"{bullet('Bot', name)}\n"
                    f"{bullet('Kind', res.get('kind', 'auto-detect'))}",
                    parse_mode="HTML"
                )
                # Show My Bots menu
                bots = list_user_bots(uid)
                ...
                bot.send_message(m.chat.id, cap + FOOTER, parse_mode="HTML", reply_markup=kb)
            else:
                bot.send_message(
                    m.chat.id,
            ...
            bot.send_message(
                m.chat.id,
                f"{G['no']} {sc('Auto-start error')}: <code>{esc(str(e))}</code>",
                parse_mode="HTML"
            )

    threading.Thread(target=_bg_start, args=(doc_db,), daemon=True).start()
    # ── auto-start the freshly uploaded bot ──────────────────────
 # ── auto-start the freshly uploaded bot ─────────────────────def _bg_start(doc: Dict[str, Any]) -> None:
    try:
        res = start_child(doc)
        if res.get("ok"):
            bot.send_message(
...
def action_payment_approve(call: types.CallbackQuery, pid: str) -> None:
    ...
    ack(call, "Approving payment...")  # ← Changed from ()
    pay["status"] = "approved"
...
def action_payment_reject(call: types.CallbackQuery, pid: str) -> None:
    ...
    if pay.get("status") in ("approved", "rejected"):
        ack(call, f"Already {pay['status']}."); return
    ack(call, "Rejecting payment...")  # ← Changed from ()
    pay["status"] = "rejected"
...
_HELP_PAGES = {
    "main": {
        "title": "📚 Help Centre",
        "text": (
            "Welcome to <b>Premium Hosting Bot</b>\n\n"
            "<b>Quick Start</b>\n"
...
def _run_diagnostics():
    ...
    report = {
        ...
        "node_found": bool(shutil.which("node")) or bool(shutil.which("nodejs")),  # ← ADD THIS
...
def _send_progress_then_captcha(chat_id: int, uid: int) -> None:
    ...
    if msg_id is not None:
        try:
            bot.edit_message_text(
                f"<b>{G['shield']} {sc(' complete')}\u2026 {sc('solve captcha below')} \u2193</b>",
...
def render_referral(call: types.CallbackQuery) -> None:
    ...
    try:
        ...
        link = f"https://t.me/PREMIUM_VPS_BOT_HOSTING_ROBOT?start={uid}"
    cap = (
        f"<b>{G['users']} {sc('Referral')}</b>\n"
        f"{G['div_eq']}\n"
        f"{bullet('Your link', link)}\n"
...
def callback_handler(call):  # Assuming this is the function name
    if data == "adm_install_node":
        if not is_owner(call.from_user.id):
            ack(call, "Owner only")
            return
        ack(call, "Installing Node.js...")
        def _bg():
            res = _install_nodejs_manual()
            try:
                bot.send_message(call.from_user.id,
                    f"<b>{'✅' if res.get('ok') else '❌'} Node.js Install</b>\n"
                    f"{bullet('Result', res.get('message', res.get('error', 'Unknown')))}",
                    parse_mode="HTML")
            except Exception:
                pass
        threading.Thread(target=_bg, daemon=True).start()
